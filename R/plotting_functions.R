...
#' @param add_zero_line boolean value indicating whether to add horizontal line
#' at zero on Y axis. Defaults to TRUE.
#' @param margins four value vector for margin sizes (bottom, left, top,
#' right).Defaults to c(5.1, 4.1, 4.1, 8)
#' @param legend_cex scaling value for legend size. Defaults to 1.
plot_values_by_month <- function(data, month_column = "month",
                                 y_axis_label = "£",
  ...
  # Add legend
  legend(
    x = x_ticks[length(x_ticks)] + 0.05, y = y_limits[2],
...
plot_values_bar_chart <- function(values, names, ...) {
