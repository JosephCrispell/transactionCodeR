...
#' @return Returns strings without hidden characters or non-alphanumeric
#'         characters
...
#'
#' Given transactions dataframe with "Type" column indicating whether
#' classified. This function finds unclassified transactions, sums and
#' counts them.
...
#' @param in_column name of column in transactions dataframe containing
#'                  money going in (credit)
#' @param out_column name of column in transactions dataframe containing
#'                   money going out (debit)
#' @param description_column column containing transaction description
#' @return Returns dataframe of unique transaction description, their cost and
#'         number of appearances
summarise_unclassified <- function(transactions,
                                   in_column,
                                   out_column,
                                   description_column) {
...
load_transaction_coding <- function(transactions_coding_df,
  ...
  if (column_names[1] != "Type" || column_names[2] != "Patterns" ||
    ncol(transactions_coding_df) != 2) {
    stop(paste0(
      "ERROR! Input transactions coding dataframe doesn't expected structure.",
      "Should have two columns", "entitled \"Type\" and \"Patterns\"!"
    ))
  }
...
#' @param transaction_coding list of pattern vectors defining transaction
#'                           types
...
classify_transaction <- function(description, transaction_coding) {
  ...
  if (is.character(description) == FALSE) {
    stop(
      "ERROR! The transaction description provided is not in the ",
      "correct format. Expecting string!"
    )
  }
...
#'
#' Classifies types for transaction descriptions type using a key word
#' dictionary
#' (list) describing key words (patterns) for different transaction types
...
#' @param transactions dataframe containing transaction details
#' @param in_column name of column in transactions dataframe containing money
#'                  going in (credit)
#' @param out_column name of column in transactions dataframe containing money
#'                   going out (debit)
#' @param grouping_columns vector of columns to group by
#' @param calculate_average boolean value to determine whether to add row at
#'                          bottom for averages. Defaults to TRUE
#' @param calculate_difference boolean value to determine whether to calculate
#'                             difference between in and out for each row.
#'                             Defaults to TRUE
#' @param ... additional parameters to send to aggregate
#' @return Returns of dataframe recording total money in and out by month. If
...
summarise_transactions <- function(transactions, in_column,
  ...
  if (calculate_difference) {
    summary_stats$difference <- summary_stats[, in_column] -
      summary_stats[, out_column]
  }
